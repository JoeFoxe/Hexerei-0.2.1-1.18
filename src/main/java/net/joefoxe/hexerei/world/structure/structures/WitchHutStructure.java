...
public class WitchHutStructure extends Structure {

    // A custom codec that changes the size limit for our code_structure_sky_fan.json's config to not be capped at 7.
    // With this, we can have a structure with a size limit up to 30 if we want to have extremely long branches of pieces in the structure.
    public static final MapCodec<WitchHutStructure> CODEC = RecordCodecBuilder.<WitchHutStructure>mapCodec(instance ->
            instance.group(WitchHutStructure.settingsCodec(instance),
            ...
            ).apply(instance, WitchHutStructure::new));
    ...
    public WitchHutStructure(Structure.StructureSettings config,
    ...
    public Optional<Structure.GenerationStub> findGenerationPoint(Structure.GenerationContext context) {
        ...
        if (!WitchHutStructure.extraSpawningChecks(context)) {
    ...
    public StructureType<?> type() {
        return ModStructures.WITCH_HUT.get(); // Helps the game know how to turn this structure back to json to save to chunks
    }
}