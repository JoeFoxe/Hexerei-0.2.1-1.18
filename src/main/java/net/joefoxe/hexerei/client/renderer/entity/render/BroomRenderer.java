...
public class BroomRenderer extends EntityRenderer<BroomEntity>
{
    protected static final ResourceLocation TEXTURE =
            HexereiUtil.getResource("textures/entity/broom.png");
    ...
    public void render(BroomEntity entityIn, float entityYaw, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn) {
        ...
        if (!Mth.equal(f2, 0.0F)) {
            matrixStackIn.mulPose((new Quaternionf()).setAngleAxis(f2 * ((float)Math.PI / 180F), 1.0F, 0.0F, 1.0F));
        }
        ...
        if(entityIn.itemHandler.getStackInSlot(2).is(HexereiTags.Items.BROOM_BRUSH)) {
            ...
            if(brushStack.getItem() instanceof BroomBrushItem brushItem && brushItem.model != null){
                ...
                broomBrushModel.renderToBuffer(matrixStackIn, brushVertexConsumer, light, OverlayTexture.NO_OVERLAY, HexereiUtil.getColorValueAlpha(1.0F, 1.0F, 1.0F, 1.0F));
        ...
        if(entityIn.itemHandler.getStackInSlot(1).is(HexereiTags.Items.ALL_SATCHELS)) {
            if(satchelStack.getItem() instanceof BroomAttachmentItem satchelItem && satchelItem.model != null){
                ...
                satchelModel.renderToBuffer(matrixStackIn, vertexConsumer, packedLightIn, OverlayTexture.NO_OVERLAY, HexereiUtil.getColorValueAlpha(1.0F, 1.0F, 1.0F, 1.0F));
        ...
        if(miscStack.is(HexereiTags.Items.BROOM_MISC)) {
            if(miscStack.getItem() instanceof KeychainItem keychainItem && keychainItem.model != null)
            {
                ...
                matrixStackIn.mulPose(Axis.ZP.rotationDegrees(-deltaMovementY * 20f));
                matrixStackIn.mulPose(Axis.YP.rotationDegrees((float)(Math.atan2(deltaMovementZ, deltaMovementX)/(2*Math.PI)*360) - entityYaw));

                if(entityIn.selfItem != null && ItemStack.isSameItemSameComponents(entityIn.selfItem, Hexerei.proxy.getPlayer().getMainHandItem()) && Minecraft.getInstance().options.getCameraType().isFirstPerson())
                {
                ...
                matrixStackIn.mulPose(Axis.ZP.rotationDegrees(((float)Mth.length(deltaMovementX, deltaMovementZ)) * 50f));
                ...
                matrixStackIn.mulPose(Axis.ZP.rotationDegrees(((float)Mth.length(deltaMovementX, deltaMovementZ)) * -20f));
                ...
                if(miscStack.has(DataComponents.CUSTOM_DATA))
                    ContainerHelper.loadAllItems(miscStack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag(), items, entityIn.level().registryAccess());
                if(items.get(0).getItem() instanceof BroomItem) {
                    matrixStackIn.scale(0.45f, 0.45f, 0.45f);
                }
                else
                    matrixStackIn.scale(0.25f,0.25f,0.25f);
                renderItem(items.get(0), entityIn.level(), matrixStackIn, bufferIn, packedLightIn);
            ...
            else if(miscStack.is(ModItems.BROOM_NETHERITE_TIP.get()))
            {
                if(miscStack.getItem() instanceof BroomAttachmentItem miscItem && miscItem.model != null) {
                    ...
                    renderItem(new ItemStack(ModItems.SELENITE_SHARD.get()), entityIn.level(), matrixStackIn, bufferIn, light);
                }
            }
            else if(miscStack.is(ModItems.BROOM_WATERPROOF_TIP.get()))
            {
                if(miscStack.getItem() instanceof BroomAttachmentItem miscItem && miscItem.model != null) {
                    ...
                    renderItem(new ItemStack(Items.CONDUIT), entityIn.level(), matrixStackIn, bufferIn, light);
                }
            }
            else
            {
